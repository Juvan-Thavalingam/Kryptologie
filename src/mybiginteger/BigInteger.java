 ...
 * BigIntegers are made as large as necessary to accommofdate the results of an
 * operation.
 * <p>
...
public class BigInteger
  ...
  public BigInteger myModPow(BigInteger exponent, BigInteger m) {
    ...
    for (int i = 0; i < exponent.bitLength(); i++) {
      if (exponent.testBit(i)) {
        BigInteger sqr = base;
        for (int j = 0; j < i; j++) {
          sqr = sqr.multiply(sqr).mod(m);
        }
        result = result.multiply(sqr).mod(m);
      }
    }
    return result;
  }
  ...
  public BigInteger myModPow(BigInteger exponent, BigInteger p, BigInteger q) {
    BigInteger r1 = p.subtract(BigInteger.ONE);
    BigInteger r2 = q.subtract(BigInteger.ONE);

    BigInteger a1 = this.modPow(exponent.mod(r1), p);
    BigInteger a2 = this.modPow(exponent.mod(r2), q);

    BigInteger u1 = q.modInverse(p);
    BigInteger u2 = p.modInverse(q);

    BigInteger x = (a1.multiply(u1).multiply(q)).add(a2.multiply(u2).multiply(p));
  ...
  public BigInteger mySqrtFloor() {
    String thisInBitString = this.toString(2);
    int mid = thisInBitString.length() / 2;
    String firstHalf = thisInBitString.substring(0, mid);

    BigInteger x_Zero = new BigInteger(firstHalf, 2);
    BigInteger x_n = BigInteger.ONE;
    BigInteger x_nPlusOne = x_Zero;
    BigInteger two = new BigInteger(2);

    do {
      x_n = x_nPlusOne;
      x_nPlusOne = x_n.square().add(this).divide(x_n.multiply(two));
    } while (x_n.compareTo(x_nPlusOne) != 0);

    BigInteger testSquare = x_nPlusOne.square();
    while (testSquare.compareTo(this) == -1) {
      x_nPlusOne = x_nPlusOne.add(ONE);
      testSquare = x_nPlusOne.square();
    }
    while (testSquare.compareTo(this) == 1) {
      x_nPlusOne = x_nPlusOne.subtract(ONE);
      testSquare = x_nPlusOne.square();
    }
  ...
  public BigInteger myModSqrt(BigInteger p) {
    ...
    BigInteger e_1 = p.subtract(ONE).divide(TWO);
    BigInteger e_2 = p.subtract(ONE);

    BigInteger testRes = this.modPow(e_1, p);
    if (testRes.equals(p.subtract(ONE).mod(p))) {
      return res;
    }
    ...
    // this.modPow(p.add(ONE).divide(FOUR), p);
    // }
    ...
    BigInteger h = ZERO;
    do {
      h = new BigInteger(p.bitLength(), src).mod(p);
      testRes = h.modPow(e_1, p);
    } while (testRes.equals(ONE) || h.equals(ZERO));

    BigInteger c = this.modPow(e_1, p).multiply(h.modPow(e_2, p)).mod(p);

    while (e_1.mod(TWO).equals(ZERO)) {
      e_1 = e_1.divide(TWO);
      e_2 = e_2.divide(TWO);

      c = this.modPow(e_1, p).multiply(h.modPow(e_2, p)).mod(p);
      if (c.equals(ONE.negate().mod(p))) {
        e_2 = e_2.add(p.subtract(ONE).divide(TWO));
      }
    }
    res = this.modPow(e_1.add(ONE).divide(TWO), p).multiply(h.modPow(e_2.divide(TWO), p)).mod(p);

    if (res.equals(ZERO) || res.equals(ONE) || res.equals(TWO)) {
      System.out.println("h = " + h.toString());
    }

    if (src.nextInt(2) == 0) {
      res = res.negate().add(p);
    }
  ...
  public BigInteger myPollardRho() {
    boolean isThisPrime = this.isProbablePrime(10);
    BigInteger d = BigInteger.ONE;

    Random source = new Random();
    int i = 1;
    while (!isThisPrime && (d.equals(this) || d.equals(ONE))) {
      BigInteger x_n = new BigInteger(this.bitLength(), source);
      BigInteger y_n = new BigInteger(x_n.toByteArray());
      BigInteger a = ZERO;
      while (a.equals(ZERO) || a.compareTo(this.subtract(TWO)) > 0) {
        a = new BigInteger(this.bitLength(), source).abs();
      }
      System.out.println("Starting Loop-" + i++);
      do {
        x_n = x_n.square().add(a).mod(this);
        y_n = (y_n.square().add(a)).square().add(a).mod(this);
        d = x_n.subtract(y_n).gcd(this);
      } while (d.equals(ONE));
    }
  ...
  public int findExp(BigInteger r)  
  {
    ...
    double rDB = r.doubleValue();

    double resultDB = Math.log(rDB) / Math.log(z);

    return (int) Math.floor(resultDB);
  }
  ...
  public BigInteger findFactor(BigInteger B) {
    createTableOfPrimes(B.intValue());
    BigInteger k = BigInteger.ONE;
    for (int i = 0; i < tableOfPrimes.length; ++i) {
      BigInteger prime = new BigInteger(tableOfPrimes[i]);
      int exponant = prime.findExp(B);
      k = k.multiply(prime.pow(exponant));
    }

    Random source = new Random();
    BigInteger a = BigInteger.ONE;
    BigInteger testNumber = BigInteger.ONE;
    BigInteger result = BigInteger.ONE;
    do {
      a = ONE;
      do {
        a = new BigInteger(this.bitLength(), source);
      } while (a.compareTo(this) > 0);
      testNumber = a.modPow(testNumber, this).subtract(ONE);
      result = testNumber.gcd(this);
  ...
  public static BigInteger myProbableSafePrime(int bitLength, int certainity, Random rnd) {
    ...
    BigInteger qPrimeCandidate = ONE;
    BigInteger pPrimeCandidate = ONE;
    BigInteger bigIntFive = new BigInteger(5);
    BigInteger bigIntSix = new BigInteger(6);
    int smallPrimesTest = 1;
    while (true) {
      boolean passedSmallPrimesTest = true;

      int tableEntry = 0;
      for (int i = 1; i < tableOfPrimes.length; ++i) {
        tableEntry = tableOfPrimes[i];
        if (tableEntry == 0) {
          System.out.println("Length: " + tableOfPrimes.length);
          System.out.println("Table of Primes Entry Var: " + tableEntry);
          System.out.println("Table of Primes Entry: " + tableOfPrimes[i] + ", i: " + i);
          System.out.println("Table of Primes Entry - 1: " + tableOfPrimes[i - 1]);
          tableEntry = tableOfPrimes[i];
          System.out.println("Table of Primes Entry Again: " + tableEntry + "\n");
        }

        BigInteger tableEntryBI = new BigInteger(tableEntry);
        smallPrimesTest = ((tableEntry - 1) / 2) % tableEntry;

        if (qPrimeCandidate.mod(tableEntryBI).intValue() == smallPrimesTest) {
          passedSmallPrimesTest = false;
          continue;
        } else if (tableEntryBI.compareTo(qPrimeCandidate) == 1) {
          continue;
        }
      }

      if (passedSmallPrimesTest) {
        pPrimeCandidate = qPrimeCandidate.multiply(TWO).add(ONE);
        if (pPrimeCandidate.isProbablePrime(certainity))
          break;
      }

      do {
        qPrimeCandidate = new BigInteger(bitLength, rnd);
      } while (!qPrimeCandidate.isProbablePrime(certainity));

      if (!qPrimeCandidate.mod(bigIntSix).equals(bigIntFive)) {
        continue;
      }
    }
    return pPrimeCandidate;
  }
  ...
  public BigInteger[] elliptAdd(BigInteger P_y,
    ...
    BigInteger s_P = elliptModCurve(this, a, b, p);
    BigInteger r_P = P_y.pow(2).mod(p);
    BigInteger s_Q = elliptModCurve(Q_x, a, b, p);
    BigInteger r_Q = Q_y.pow(2).mod(p);

    if ((!this.equals(p) && !Q_x.equals(p)) && (!s_P.equals(r_P) || !s_Q.equals(r_Q))) {
    ...
    if (this.equals(p)) {
      res[0] = Q_x;
      res[1] = Q_y;
    } else if (Q_x.equals(p)) {
      res[0] = this;
      res[1] = P_y;
    } else if (!this.equals(Q_x)) {
      BigInteger numerator = Q_y.subtract(P_y);
      BigInteger devisor = Q_x.subtract(this);
      BigInteger m = numerator.multiply(devisor.modInverse(p)).mod(p);
      res[0] = m.pow(2).subtract(this).subtract(Q_x).mod(p);
      res[1] = m.negate().multiply(res[0].subtract(this)).subtract(P_y).mod(p);
    } else if (this.equals(Q_x) && P_y.equals(Q_y) && (!P_y.equals(ZERO))) {
      BigInteger numerator = this.pow(2).multiply(three).add(a);
      BigInteger devisor = P_y.multiply(BigInteger.TWO);
      BigInteger m = numerator.multiply(devisor.modInverse(p)).mod(p);
      res[0] = m.pow(2).subtract(this.multiply(BigInteger.TWO)).mod(p);
      res[1] = m.negate().multiply(res[0].subtract(this)).subtract(P_y).mod(p);
    }
  ...
  public BigInteger[] elliptMultiply(BigInteger y,
    ...
    BigInteger[] res = {ZERO, ZERO};
    BigInteger s_P = elliptModCurve(this, a, b, p);
    BigInteger r_P = y.pow(2).mod(p);

    if (!this.equals(p) && !s_P.equals(r_P)) {
      throw new NumberFormatException("Punkt liegt nicht auf der Kurve!");
    }
    ...
    String factorInBinary = factor.toString(2);
    int factorBinLength = factorInBinary.length();
    for (int i = 0; i < factorBinLength; ++i) {
      char binValue = factorInBinary.charAt(factorBinLength - 1 - i);
      if (binValue != '1') {
        continue;
      }
      BigInteger[] doubling = { this, y };
      for (int j = 0; j < i; ++j) {
        doubling = doubling[0].elliptAdd(doubling[1], doubling[0], doubling[1], p, a, b);
      }
      res = res[0].elliptAdd(res[1], doubling[0], doubling[1], p, a, b);
    }

    if (isNegative) {
      res[1] = res[1].compareTo(ZERO) > 0 ? res[1].negate().add(p) : res[1].negate();
    }

    return res;
  }

  private BigInteger elliptModCurve(BigInteger x, BigInteger a, BigInteger b, BigInteger p) {
