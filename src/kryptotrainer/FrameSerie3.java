...
// ContentPane
  ...
  private void doDatenbankVerschl() {
      ...
      for (int i = 0; i < schluessel.length; ++i) {
          BigInteger keyC = BigInteger.probablePrime(keyBitLength, random);
          boolean conflict = true;
          while (conflict) {
              for (int j = 0; j < schluessel.length; ++j) {
                  conflict = schluessel[j].compareTo(keyC) == 0;
                  if (conflict) {
                      keyC = BigInteger.probablePrime(keyBitLength, random);
                      break;
                  }
              }
          }
    ...
    // Datenbank verschlüsseln
      ...
      BigInteger[] modulo_i = new BigInteger[5];
      BigInteger[] u_i = new BigInteger[5];
      for (int i = 0; i < schluessel.length; ++i) {
          m = m.multiply(schluessel[i]);
      }

      for (int i = 0; i < modulo_i.length; ++i) {
          modulo_i[i] = m.divide(schluessel[i]);
          u_i[i] = modulo_i[i].modInverse(schluessel[i]);
      }

      for (int i = 0; i < schluessel.length; ++i) {
          datenbankVerschl = datenbankVerschl.add(datenSatzUnverschl[i].multiply(u_i[i].multiply(modulo_i[i])));
